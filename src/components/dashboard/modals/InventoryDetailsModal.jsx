import { AlignLeft, CalendarDays, Package, X } from 'lucide-react';
        ...
        <div className="mb-6 border-b-4 border-[#0a0a0a] pb-4 pr-8">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <span className="bg-[#0a0a0a] px-2 py-1 font-tech text-xs font-bold uppercase tracking-widest text-[#fcfcfc]">
              {selectedInventoryDetails.project}
            </span>
            <span
              className={`border-2 px-2 py-0.5 font-tech text-xs font-bold uppercase tracking-widest ${
                selectedInventoryDetails.type === 'Consumível'
                  ? 'border-[#888888] text-[#888888]'
                  : 'border-[#0a0a0a] bg-[#e5e5e5] text-[#0a0a0a]'
              }`}
            >
              {selectedInventoryDetails.type}
            </span>
          </div>
          <h2 className="font-display text-4xl font-bold uppercase leading-tight text-[#0a0a0a] md:text-5xl">
            {selectedInventoryDetails.name}
          </h2>
        </div>

        <div className="space-y-8">
          <div className="flex items-start gap-4">
            {selectedInventoryDetails.photo ? (
              <img
                src={selectedInventoryDetails.photo}
                alt={selectedInventoryDetails.name}
                className="h-24 w-24 border-2 border-[#0a0a0a] bg-[#e5e5e5] object-cover"
              />
            ) : (
              <div className="flex h-24 w-24 items-center justify-center border-2 border-[#0a0a0a] bg-[#e5e5e5]">
            ...
            <div className="grid flex-1 grid-cols-1 gap-4 md:grid-cols-2">
              <div className="border-2 border-[#e5e5e5] p-4">
                ...
                <p className="font-display text-3xl font-bold uppercase text-[#0a0a0a]">
                  {selectedInventoryDetails.quantity}
                </p>
              </div>
              <div className="border-2 border-[#e5e5e5] p-4">
                <h4 className="mb-2 font-tech text-xs font-bold uppercase tracking-widest text-[#888888]">
                ...
                <p className="font-tech text-sm font-bold uppercase tracking-widest text-[#0a0a0a]">
                  {formatCurrency(selectedInventoryDetails.value)}
                </p>
              </div>
              <div className="border-2 border-[#e5e5e5] p-4 md:col-span-2">
                <h4 className="mb-2 flex items-center gap-2 font-tech text-xs font-bold uppercase tracking-widest text-[#888888]">
                  <CalendarDays className="h-4 w-4" /> Data de Compra
                </h4>
                <p className="font-tech text-sm font-bold uppercase tracking-widest text-[#0a0a0a]">
                  {formatDate(selectedInventoryDetails.purchaseDate)}
                </p>
              </div>
            </div>
          </div>
